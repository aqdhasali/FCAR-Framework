...
def _extract_cat_weights(pipe, mutable_cat_cols):
    ...
    names.  String-splitting breaks on columns or category values that contain
    underscores (e.g. 'work_class_Private' or 'Exec-managerial').  Config
    column names are matched to sklearn's internal names by normalising
    hyphens and underscores before comparison.
    """
    pre = pipe.named_steps["pre"]
    clf = pipe.named_steps["clf"]
    ...
    all_cat_cols = list(pre.transformers_[1][2])  # sklearn's actual column names

    def _normalize(name: str) -> str:
        return name.replace("-", "_").replace(" ", "_").lower()

    # Map each sklearn column name to the corresponding config column name
...
def solve_recourse_mip(
    ...
    if tc not in (TerminationCondition.optimal, TerminationCondition.maxTimeLimit):
        # Infeasible, unbounded, or error — return sentinel value
    ...
    try:
        ...
        # Time limit reached without a feasible point, or value extraction failed
        return x0.copy(), float("inf")
